      ...
      <div className="max-w-7xl mx-auto px-8 py-12">
        ...
        {mode === 'menu' && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-12 shadow-xl">
            <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-8 text-center">What would you like to study?</h2>
            <div className="grid grid-cols-3 gap-6">
              <button
                onClick={fetchNewCharacters}
                className="p-8 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800 rounded-xl hover:shadow-lg transition border-2 border-blue-200 dark:border-blue-700"
              >
                <div className="text-4xl mb-3">✨</div>
                <h3 className="text-xl font-bold text-blue-900 dark:text-blue-100 mb-2">New Characters</h3>
                <p className="text-sm text-blue-700 dark:text-blue-300">Learn new characters in order of frequency</p>
              </button>

              <button
                onClick={fetchReviewCharacters}
                className="p-8 bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900 dark:to-purple-800 rounded-xl hover:shadow-lg transition border-2 border-purple-200 dark:border-purple-700"
              >
                <div className="text-4xl mb-3">🔄</div>
                <h3 className="text-xl font-bold text-purple-900 dark:text-purple-100 mb-2">Review</h3>
                <p className="text-sm text-purple-700 dark:text-purple-300">Review characters you're currently learning</p>
              </button>

              <button
                onClick={fetchSelectCharacters}
                className="p-8 bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900 dark:to-green-800 rounded-xl hover:shadow-lg transition border-2 border-green-200 dark:border-green-700"
              >
                <div className="text-4xl mb-3">🎯</div>
                <h3 className="text-xl font-bold text-green-900 dark:text-green-100 mb-2">Select</h3>
                <p className="text-sm text-green-700 dark:text-green-300">Choose specific characters to practice</p>
              </button>
            </div>
          </div>
        )}
        ...
        {mode === 'select' && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-xl">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Select Characters to Study</h2>
            <div className="grid grid-cols-4 gap-4 mb-8 max-h-96 overflow-y-auto">
              {characters.map((char, idx) => (
                ...
                >
                  <div className="text-3xl font-bold text-red-600 dark:text-red-300 mb-2">{char.character}</div>
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 px-6 py-3 bg-red-600 text-white font-bold rounded-lg hover:bg-red-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Start Practice ({selectedCharacters.length} selected)
              </button>
              <button
                onClick={() => setMode('menu')}
                className="px-6 py-3 border-2 border-red-600 text-red-600 dark:text-red-400 dark:border-red-400 font-bold rounded-lg hover:bg-red-50 dark:hover:bg-slate-700 transition"
              >
                Back
              </button>
            </div>
        ...
        {mode === 'practice' && currentCharacter && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-8">
              {/* Character Info */}
              <div>
                <div className="bg-gradient-to-br from-red-100 to-red-50 dark:from-red-900 dark:to-red-800 rounded-xl p-12 mb-6 border-4 border-red-200 dark:border-red-700">
                  <div className="text-8xl font-bold text-red-600 dark:text-red-300 text-center">{currentCharacter.character}</div>
                </div>
                
                <div className="space-y-4">
                  <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-4">
                    <p className="text-xs text-slate-600 dark:text-slate-400 uppercase mb-1">Pinyin</p>
                    <p className="text-2xl font-bold text-slate-900 dark:text-white">{currentCharacter.pinyin}</p>
                  </div>
                  
                  <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-4">
                    <p className="text-xs text-slate-600 dark:text-slate-400 uppercase mb-1">Meaning</p>
                    <p className="text-2xl font-bold text-slate-900 dark:text-white">{currentCharacter.meaning}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col">
                <p className="text-sm text-slate-600 dark:text-slate-400 mb-4 font-semibold">Draw the character below:</p>
                <div className="bg-gray-100 dark:bg-slate-700 rounded-xl p-4 flex-1 flex items-center justify-center mb-6">
                  <HanziCanvas 
                ...
                <button
                  ...
                  className={`px-6 py-4 font-bold rounded-lg transition ${
                    isCharacterComplete
